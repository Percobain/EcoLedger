const API_BASE_URL = "https://ecoledger.onrender.com/api";
...
export const generateImpactMetrics = async (projectData) => {
    try {
        console.log(
            "🌱 Generating impact metrics for project:",
            projectData.name
        );

        const response = await fetch(
            `${API_BASE_URL}/impact-metrics/generate`,
            {
                method: "POST",
                headers: {
        ...
        if (!result.success) {
            throw new Error(
                result.error || "Failed to generate impact metrics"
            );
        }

        console.log(
            "✅ Impact metrics generated successfully:",
            result.metrics
        );
...
export const generateBatchImpactMetrics = async (projects) => {
    try {
        console.log(
            `🌱 Generating impact metrics for ${projects.length} projects`
        );

        const response = await fetch(
            `${API_BASE_URL}/impact-metrics/generate-batch`,
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
        ...
        if (!result.success) {
            throw new Error(
                result.error || "Failed to generate batch impact metrics"
            );
        }
...
export const generateFallbackMetrics = (projectData) => {
    const targetPlants =
        projectData.project_details?.target_plants ||
        projectData.about_project?.target_plants ||
        projectData.attributes?.find(
            (attr) => attr.trait_type === "Target Plants"
        )?.value ||
        0;

    const ecosystem =
        projectData.project_details?.ecosystem ||
        projectData.about_project?.ecosystem ||
        projectData.attributes?.find((attr) => attr.trait_type === "Ecosystem")
            ?.value ||
        "Unknown";

    const communityImpact =
        projectData.project_details?.community_impact ||
    ...
    const biodiversityScore =
        projectData.project_details?.biodiversity_score ||
    ...
    const areaHectares =
        parseFloat(projectData.project_details?.total_area) ||
        targetPlants / 1000;
